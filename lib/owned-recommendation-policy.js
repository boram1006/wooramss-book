...
function resolveBookThemes(book, limit = 3) {
  ...
  const title = String(book?.fields?.['제목'] || '').toLowerCase();
  ...
  if (/도전|용기/.test(title)) ownedBookHints.push('용기·도전');
  if (/마음|감정|기분/.test(title)) ownedBookHints.push('감정 이해');
  if (/여행|모험|탐험/.test(title)) ownedBookHints.push('모험·탐험');
  ...
  return [...new Set([...ownedBookHints, ...metadataThemes, ...contentThemes])].slice(0, limit);
}

function cleanOwnedFallbackDescription(value) {
  return String(value || '')
    .replace(/^\s*무엇보다도 재미있는 것은[,]?\s*/, '')
    .replace(/다는 것([.!?。])?\s*$/, '다는 점이 눈에 띄어요.')
    .trim();
}
...
function buildReadingSignal(evidence, primaryTheme) {
  const values = evidence || [];
  const countFor = pattern => {
    const match = values.find(item => pattern.test(item))?.match(/(\d+)회/);
    return match ? Number(match[1]) : 0;
  };
  return {
    theme: primaryTheme || null,
    repeated: countFor(/테마 완독/) >= 2,
    focused: countFor(/테마 집중/) >= 1,
    askedQuestions: countFor(/테마 질문 많음/) >= 1,
    explicitInterest: values.some(item => /^명시 관심사 일치:/.test(item))
  };
}

function hasRecommendationAuditLanguage(value) {
  return /완독|집중도|집중 기록|독서 기록|기록이 있|기록과|주제 책을 최근|최근 .{0,20}(?:번|회).{0,12}읽/.test(String(value || ''));
}
