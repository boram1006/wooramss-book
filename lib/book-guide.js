...
const GUIDE_ITEM_SCHEMA = {
  ...
  required: ['key', 'ageRange', 'parentGuide', 'activities'],
...
const SYSTEM_PROMPT = `역할: 당신은 3~5세 유아의 문해력 발달, 상호작용적 함께 읽기, 놀이 중심 학습을 돕는 한국어 부모 코치입니다.
...
- parentGuide: 한국어 150~230자. "함께 볼 점: ... 질문: ① ... ② ... 반응: ..." 순서로 씁니다. 질문은 정확히 2개이며, 첫 질문은 그림·사건을 아이 말로 설명하게 하고 둘째 질문은 예측·선택·아이 경험 중 하나로 연결합니다. 부모가 답을 재촉하거나 고치지 않고 기다린 뒤 아이의 말을 한두 단어 확장해 되돌려주도록 안내합니다.
...
- 따뜻하지만 추상적이지 않은 존댓말을 쓰고, 마크다운과 줄바꿈 없이 각 값을 완결된 문장으로 작성합니다.
...
function isUsableGuide(guide) {
  return Boolean(guide.ageRange) && Boolean(guide.parentGuide) && Boolean(guide.activities);
}
...
module.exports = { EMPTY_GUIDE, SYSTEM_PROMPT, generateBookGuide, generateBookGuides, sanitizeGuide };
