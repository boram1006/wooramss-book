...
- 여러 책을 한 번에 받으면 적합성이 비슷한 경우 덜 사용한 전략을 골라 전략을 고르게 분산합니다. 같은 전략을 세 권 연속 쓰지 않습니다.
...
- question1, question2: 아이가 자기 말로 답할 수 있는 서로 다른 기능의 열린 질문을 각각 12~28자의 짧은 한 문장으로 씁니다. 관찰, 예측, 회상, 근거 찾기, 낱말, 인물 관점, 경험 연결 중 선택한 전략에 맞는 두 기능을 고릅니다. 한 질문에는 의문을 하나만 담습니다.
...
- "시키세요", "유도하세요"보다 아이가 고르고 말할 여지를 주는 표현을 씁니다.
...
function sanitizeGeneratedGuide(value) {
  const focus = cleanSection(value?.focus, 160);
  const question1 = cleanSection(value?.question1, 120);
  const question2 = cleanSection(value?.question2, 120);
  const response = cleanSection(value?.response, 200);
  return {
    ageRange: cleanText(value?.ageRange, 30),
...
async function generateBookGuides(inputBooks, options = {}) {
  ...
  return books.map(book => {
    ...
    if (!isUsableGuide(guide)) return { key: book.key, ...EMPTY_GUIDE };
    const { _parts, ...value } = guide;
    return { key: book.key, ...value };
  });
}
