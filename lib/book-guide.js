...
const STRATEGY_SIGNALS = Object.freeze({
  '그림 단서 찾기': Object.freeze([
    [/글\s*없는|숨은\s*그림|그림\s*속\s*단서/u, 7],
    [/색감|색채|문양|명화|시각적|역동적인\s*이미지|그림으로\s*(?:전개|구성|표현)/u, 4]
  ]),
  '예측하고 확인하기': Object.freeze([
    [/수수께끼|반전|예상치|다음\s*장면|결말을\s*상상|대체\s*무엇|무슨\s*일이/u, 6],
    [/갑자기|과연|수상한|비밀|궁금|뜻밖|이상한\s*일|벌어지|나타난다|맞닥뜨/u, 3]
  ]),
  '순서 다시 말하기': Object.freeze([
    [/만드는\s*(?:과정|법)|만들기\s*(?:과정|법)|떡을\s*만드|전\s*과정|단계별|차례대로|순서대로|완성하는\s*과정/u, 7],
    [/여정|여행|모험|길을\s*나서|떠나|바깥세상|밖으로\s*나/u, 5],
    [/준비를\s*하|여러\s*사건|에피소드|처음부터\s*끝|꼬리에\s*꼬리|이어져\s*가|누적|반복\s*구조|과정/u, 3]
  ]),
  '감정과 관점 나누기': Object.freeze([
    [/감정|마음|기분|서운|두려|불안|공감/u, 5],
    [/감정의\s*변화|마음의\s*변화|관점|입장|속마음/u, 6],
    [/용기|화해/u, 3]
  ]),
  '낱말과 개념 익히기': Object.freeze([
    [/원리|생태|과학|지식|정보책|도감|몸의\s*기관/u, 6],
    [/구조|기능|개념|어떻게\s*(?:생기|움직|작동)/u, 5]
  ]),
  '소리와 말놀이': Object.freeze([
    [/의성어|의태어|말놀이|운율|반복구|ㄱㄴㄷ|랄랄라|냐무냐무|우지끈|푸시시/u, 7],
    [/입말|말의\s*리듬|소리\s*(?:표현|가\s*들려|를\s*내)|리듬감|자장가/u, 4]
  ]),
  '경험과 장면 잇기': Object.freeze([
    [/일상|우리\s*집|유치원|등원|잠자리|목욕|식사/u, 5],
    [/가족|엄마|아빠|친구|이웃|생활\s*경험/u, 3]
  ]),
  '인물 되어 말하기': Object.freeze([
    [/대화|말을\s*주고받|역할|인물이\s*되어/u, 6],
    [/다툼|갈등|문제\s*해결|부탁|사과|협상/u, 4]
  ])
});

const STRATEGY_TIE_PRIORITY = Object.freeze([
  '낱말과 개념 익히기',
  '소리와 말놀이',
  '순서 다시 말하기',
  '감정과 관점 나누기',
...
const QUALITY_FEEDBACK = Object.freeze({
  ...
  directive_response: '시키기·유도하기·해 달라 하기 표현을 쓰지 마세요',
  ...
  wrong_book_anchor_particle: '『bookAnchor』 뒤 조사를 마지막 글자의 받침에 맞게 고치세요',
  meta_response_wording: '비교한다고 말해 보세요처럼 행동을 설명하지 말고 장면을 직접 비교해 보세요',
...
function rankInteractionStrategies(book) {
  ...
  const fields = [
    { value: title, multiplier: 1.8 },
    { value: description, multiplier: 1 },
    { value: features, multiplier: 1.35 }
  ];

  return INTERACTION_STRATEGIES.map(strategy => {
    let score = 0;
    let evidenceCount = 0;
    for (const [pattern, weight] of STRATEGY_SIGNALS[strategy]) {
      for (const field of fields) {
        if (!field.value || !pattern.test(field.value)) continue;
        score += weight * field.multiplier;
        evidenceCount += 1;
      }
    }
    return { strategy, score, evidenceCount };
  }).sort((a, b) => (
    b.score - a.score
    || b.evidenceCount - a.evidenceCount
    || STRATEGY_TIE_PRIORITY.indexOf(a.strategy) - STRATEGY_TIE_PRIORITY.indexOf(b.strategy)
  ));
}

function selectInteractionStrategy(book) {
  const description = cleanText(book?.description || book?.설명, 1600);
  const ranked = rankInteractionStrategies(book);
  if (ranked[0]?.score > 0) return ranked[0].strategy;
...
- '소리와 말놀이' 전략에서는 추상적인 생각·마음보다 제목·소개에 실제로 나온 의성어·의태어·반복구를 bookAnchor로 고릅니다.
...
- 『bookAnchor』 뒤의 이/가, 은/는, 을/를, 과/와는 마지막 글자의 받침에 맞춥니다. "비교한다고 말해 보세요"처럼 부모 행동을 설명하지 말고 실제로 무엇을 짚고 비교할지 직접 씁니다.
...
- "시키세요", "유도하세요", "말하게 하세요", "보게 하고", "보여 달라 해 보세요"보다 아이가 고르고 말할 여지를 주는 표현을 씁니다.
...
function koreanFinalConsonant(value) {
  const chars = Array.from(String(value || '')).reverse();
  const syllable = chars.find(char => {
...
function hasWrongAnchorParticle(response, bookAnchor) {
  if (!bookAnchor) return false;
  const marker = `『${bookAnchor}』`;
  const index = response.indexOf(marker);
  if (index < 0) return false;
  const particle = response.slice(index + marker.length, index + marker.length + 1);
  const hasFinal = koreanFinalConsonant(bookAnchor);
  if (hasFinal === null) return false;
  const expected = hasFinal
    ? { 이: true, 은: true, 을: true, 과: true }
    : { 가: true, 는: true, 를: true, 와: true };
  return /[이가은는을를과와]/u.test(particle) && !expected[particle];
}
...
function assessGuideQuality(guide) {
  ...
  if (/(?!함께\s*(?:하|해))[가-힣]+게\s*(?:하|해)|달라\s*해|해\s*달라|시키|유도/u.test(response)) issues.push('directive_response');
  ...
  if (hasWrongAnchorParticle(response, bookAnchor)) issues.push('wrong_book_anchor_particle');
  if (/비교한다고\s*말해|확인한다고\s*말해/u.test(response)) issues.push('meta_response_wording');
