...
const GUIDE_ITEM_SCHEMA = {
  ...
  properties: {
    ...
    interactionStrategy: { type: 'string', enum: INTERACTION_STRATEGIES },
    focus: { type: 'string' },
  ...
  required: ['key', 'ageRange', 'interactionStrategy', 'focus', 'question1', 'question2', 'response', 'activities'],
...
목표: 책마다 내용과 표현 방식에 가장 잘 맞는 상호작용 하나를 골라, 그 책에서만 쓸 수 있는 부모 가이드와 연계 놀이를 만드세요. 아이가 이야기하고 부모는 아이의 관심을 따라 구체적으로 반응합니다.
...
- 책 적합성이 전략 분산보다 우선입니다. 소개에 의성어·의태어·운율이 명시되면 '소리와 말놀이', 구조·기능·생태·지식을 설명하면 '낱말과 개념 익히기', 만드는 과정·여정·누적 순서가 명시되면 '순서 다시 말하기'를 우선 검토합니다.
- 여러 책을 한 번에 받으면 적합성이 같은 경우에만 덜 사용한 전략을 골라 분산합니다. 다른 전략을 쓰기 위해 책과 덜 맞는 전략을 고르지 말고, 같은 전략을 세 권 연속 쓰지 않습니다.

출력 기준:
- ageRange: 책 자체의 권장 연령을 "3-5세" 형식으로 추정합니다. 아이의 현재 나이와 혼동하지 않습니다.
- interactionStrategy: 위 8개 전략 중 정확히 하나를 씁니다.
- focus: 부모가 함께 살펴볼 책 속 대상·장면·표현을 20~40자로 씁니다.
- question1, question2: 아이가 자기 말로 답할 수 있는 서로 다른 기능의 열린 질문을 각각 12~28자의 짧은 한 문장으로 씁니다. 각 질문에 '무엇·어떤·어디·누가·언제·어떻게·무슨' 중 자연스러운 의문 표현을 넣고 한 질문에는 의문을 하나만 담습니다.
...
- response: 아이의 답 다음에 부모가 할 구체적인 행동을 35~60자로 씁니다. 선택한 전략을 실제로 실행하고 focus의 구체적 대상과 연결합니다. 모든 책에 통용되는 '잠깐 기다리기', '한두 단어 덧붙이기', '말을 확장해 되돌려주기'를 쓰지 않습니다. "아이의 답을 듣고" 같은 공통 도입 대신 "다음 장을 넘겨", "표정이 보이는 곳을 짚어"처럼 책에서 곧바로 할 행동으로 시작합니다.
...
- "해볼래?", "맞아?", "보여?"처럼 예·아니요로 끝나는 질문과 선택한 전략에서 벗어난 취향 질문은 쓰지 않습니다.
...
function sanitizeGeneratedGuide(value) {
  ...
  return {
    ageRange: cleanText(value?.ageRange, 30),
    interactionStrategy: cleanText(value?.interactionStrategy, 40),
...
function assessGuideQuality(guide) {
  ...
  if (/말하게\s*해|시키세요|유도하세요/u.test(response)) issues.push('directive_response');
  ...
  if (questions.some(question => !/무엇|뭐|어떤|어디|누가|언제|어떻게|무슨|왜|어느|몇/u.test(question))) issues.push('question_not_open');
...
async function generateBookGuides(inputBooks, options = {}) {
  ...
  const generatedByKey = new Map((parsed.guides || []).map(item => [cleanText(item.key, 80), sanitizeGeneratedGuide(item)]));
