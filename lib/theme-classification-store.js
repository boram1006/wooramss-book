...
function validateClassification(body = {}) {
  ...
  const legacyMappedTheme = String(body.mappedTheme || '').trim();
  const mappedThemes = [...new Set([
    ...(Array.isArray(body.mappedThemes) ? body.mappedThemes : []),
    ...(legacyMappedTheme ? [legacyMappedTheme] : [])
  ].map(theme => String(theme || '').trim()).filter(Boolean))].slice(0, 4);

  if (!normalizedExpression) return { error: '분류할 표현이 없습니다.' };
  if (!VALID_ACTIONS.has(status)) return { error: '지원하지 않는 처리 방식입니다.' };
  if (status === 'mapped' && (!mappedThemes.length || mappedThemes.some(theme => !THEME_CATALOG.includes(theme)))) {
    return { error: '표준 테마를 하나 이상 선택해주세요.' };
  }
  ...
  return {
    value: {
      ...
      status,
      mappedTheme: status === 'mapped' ? mappedThemes[0] : null,
      mappedThemes: status === 'mapped' ? mappedThemes : []
    }
  };
}
...
function validateResidualReopen(body = {}) {
  const bookId = String(body.bookId || '').trim();
  const themesToRemove = [...new Set((Array.isArray(body.themesToRemove) ? body.themesToRemove : [])
    .map(theme => String(theme || '').trim())
    .filter(Boolean))];
  if (!bookId) return { error: '다시 검토할 책이 없습니다.' };
  if (!themesToRemove.length || themesToRemove.some(theme => !THEME_CATALOG.includes(theme))) {
    return { error: '제거할 표준 테마를 확인해주세요.' };
  }
  return { value: { bookId, themesToRemove } };
}
...
function removeBookThemes(currentThemes, removals = []) {
  const removalSet = new Set(removals.filter(theme => THEME_CATALOG.includes(theme)));
  const current = Array.isArray(currentThemes)
    ? currentThemes.map(theme => String(theme || '').trim()).filter(Boolean)
    : String(currentThemes || '').split(/[,|;/\n]+/).map(theme => theme.trim()).filter(Boolean);
  const next = current.filter(theme => !removalSet.has(theme));
  return Array.isArray(currentThemes) ? next : next.join(',');
}
...
async function loadOpenTaxonomyResiduals(supabase, options = {}) {
  let query = supabase
    .from('taxonomy_review_residuals_v2')
    .select('*')
    .order('title', { ascending: true });
  if (!options.includeReviewed) query = query.eq('status', 'pending');
