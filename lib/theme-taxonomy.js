const THEME_GROUPS = Object.freeze([
  {
    id: 'relationships',
    label: '관계와 사회성',
    themes: ['가족', '친구·우정', '형제자매', '이웃·공동체', '배려·나눔', '다양성·존중', '의사소통']
  },
  {
    id: 'emotions',
    label: '마음과 성장',
    themes: ['감정 이해', '자존감·나다움', '성장·자립', '용기·도전', '공감·위로', '갈등·화해', '책임·자기조절']
  },
  {
    id: 'interests',
    label: '관심 세계',
    themes: ['동물·생명', '자연·계절', '환경·공존', '과학·탐구', '탈것·도시', '음식·요리', '몸·건강', '문화·전통', '책·언어', '감각·기초 개념', '사회·경제·직업']
  },
  {
    id: 'story',
    label: '이야기 경험',
...
const ALIASES = Object.freeze({
  ...
  '성장·자립': ['성장', '성장과변화', '변화와성장', '독립', '자립', '이사', '성장과독립', '성장과자립'],
  '용기·도전': ['용기', '도전', '도전정신', '극복', '두려움극복', '실패', '끈기', '인내'],
  '공감·위로': ['공감', '위로', '사랑', '애정', '마음돌봄', '치유'],
  '갈등·화해': ['갈등', '화해', '사과', '용서', '다툼', '말다툼', '충돌', '으름장', '문제해결'],
  '책임·자기조절': ['책임·자기조절'],
  ...
  '책·언어': ['책·언어'],
  '감각·기초 개념': ['감각·기초 개념'],
  '사회·경제·직업': ['사회·경제·직업', '사회문제', '경제', '직업', '공장', '백화점', '회사', '노동'],
  '일상생활': ['일상', '일상의소중함', '생활', '등원', '유치원', '어린이집', '잠', '침대', '목욕', '배변', '습관', '놀이'],
...
const observedUnclassified = new Set();
...
function listUnclassifiedThemes(value, overrides) {
  const result = [];
  for (const raw of splitThemeValues(value)) {
    const original = String(raw || '').trim();
    const normalized = compact(original);
    if (!normalized || NON_THEME_TERMS.has(normalized) || canonicalizeTheme(original, overrides) || overrides instanceof Map && overrides.has(normalized)) continue;
    if (!result.some(item => item.normalized === normalized)) {
      result.push({ raw: original, normalized });
    }
  }
  return result;
}

function observeUnclassifiedThemes(value, context = {}, logger = console.warn) {
  const source = String(context.source || 'unknown');
  const safeContext = Object.fromEntries(
    Object.entries(context).filter(([, item]) => ['string', 'number', 'boolean'].includes(typeof item))
  );

  const unclassified = listUnclassifiedThemes(value);
  for (const item of unclassified) {
    const dedupeKey = `${source}:${item.normalized}`;
    if (observedUnclassified.has(dedupeKey)) continue;
    observedUnclassified.add(dedupeKey);
    logger('[theme-taxonomy] ' + JSON.stringify({
      event: 'unclassified_theme',
      raw: item.raw,
...
function resetObservedUnclassifiedThemes() {
  observedUnclassified.clear();
}

function normalizeThemes(value, limit = 8, overrides) {
  const result = [];
  for (const raw of splitThemeValues(value)) {
    const key = compact(raw);
    if (overrides instanceof Map && overrides.has(key)) {
      const override = overrides.get(key);
      const mappedThemes = Array.isArray(override) ? override : [override];
    ...
    const canonical = canonicalizeTheme(raw, overrides);
...
function inferThemes(book, limit = 3, overrides) {
  const sourceThemes = normalizeThemes(book?.themes || book?.['테마'], limit, overrides);
