...
// 전체 재생성이 끝나면 이 일회성 경로와 키를 함께 제거한다.
const BULK_REFRESH_TOKEN = '1f2712578a127fdcd2bb57b9a0093246683edeee46354cae';
...
function debugLog(...args) {
  ...
  try {
    ...
    if (Array.isArray(bookIds)) {
      ...
      const uniqueBookIds = Array.from(new Set(bookIds.map(String).filter(Boolean))).slice(0, 10);
      ...
      if ((storedBooks || []).length !== uniqueBookIds.length) throw new Error('일부 책을 찾지 못했습니다');
